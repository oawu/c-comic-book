...
void RunCommand (char* WgetFile, char* Url, char* FileName){
  ...
  int x2=0,y2=0;
  ClearFile(FileName);
  char* Command = MergeString ("%s %s -O %s", WgetFile, Url, FileName);
  system(Command);
  free (Command);
  ...
  if (!FileIsExists (DataBase->DataBasePath)) {
    ...
    if (Result == SQLITE_OK){
      ...
      char* InsertVariable[] = {
        ...
        ,"INSERT INTO Variable (Sequence, KEY, Value) VALUES (14, 'Version',              'OA_ComicBook_2.2_F');"
...
int InterfaceCursor (int Left, int Top, char *Key, int PageItemCount, int *NowIndex, int Height, int page, int ItemCount) {
  int ReturnValue = 0;
    ...
    if (ItemList!=NULL) {
      ...
      *Choice = *Choice - 1;
      ...
      while (ItemList != NULL) {
        ...
        ITEM* Item = ItemList->GetStartItem (ItemList, NowIndex / Heigth * Heigth);
        for (int Index = 0; Index < Heigth; Index++) {
          if (Item != NULL) {
            Print ("   %5d. %15s -- %20s< %28s\n",(Index + 1) + (page * Heigth), Item->Name, Item->EnglishName, Item->Explain);
            InPageItemCount++;
            Item = Item->Next;
          }
          else {
        ...
        while (true) {
          ...
          if (NowIndex == -1)
            NowIndex = 0;
          if (NowIndex >= InPageItemCount + page * Heigth)
            NowIndex = InPageItemCount + page * Heigth - 1;

          *Choice = InterfaceCursor ( 2, Top, Interface->Symbol, InPageItemCount, &NowIndex, Heigth, page, ItemCount);

          if (((NowIndex) > InPageItemCount + page * Heigth - 1 && Item != NULL) || (NowIndex < (page) * Heigth && NowIndex != -1)) {
            page = NowIndex / Heigth;
            break;
          }
          if (*Choice != 0)
    ...
    if (ItemList!=NULL) {
      ...
      *Choice = *Choice - 1;
      ...
      while (ItemList != NULL) {
        ...
        for (int Index = 0; Index < MessageLiseCount; Index++, Heigth--) {
          Print (" %=rW %76s| %=Nw\n", MessageLise[Index]);
    ...
    if (ComicList!=NULL) {
      ...
      *Choice = *Choice - 1;
      ...
      while (ComicList != NULL) {
        ...
        COMIC* Comic = ComicList->GetStartComic (ComicList, NowIndex / Heigth * Heigth);
        for (int Index = 0; Index < Heigth; Index++) {
          if (Comic != NULL) {
            char* ComicChapterCount = MergeString ("共有 %3d 集(話)",Comic->GetThisComicChapterCount (DataBase, Comic));
            Print ("   %5d. %15s -- %20s< %28s\n",(Index + 1) + (page * Heigth), Comic->Name, Comic->Name, ComicChapterCount);
        ...
        while (true) {
          ...
          *Choice = InterfaceCursor ( 2, Top, Interface->Symbol, InPageItemCount, &NowIndex, Heigth, page, ItemCount);
          ...
          if (((NowIndex) > InPageItemCount + page * Heigth - 1 && Comic != NULL) || (NowIndex < (page) * Heigth && NowIndex != -1)) {
            page = NowIndex / Heigth;
            break;
          }
          if (*Choice != 0)
    ...
    if (ChapterList!=NULL) {
      ...
      *Choice = *Choice - 1;
      ...
      while (ChapterList != NULL) {
        ...
        CHAPTER* Chapter = ChapterList->GetStartChapter (ChapterList, NowIndex / Heigth * Heigth);
        for (int Index = 0; Index < Heigth; Index++) {
          if (Chapter != NULL) {
            Print ("   %5d.      第 %4s 章節 -- Chapter. %10s<         共有 %3s 張漫畫圖片\n",(Index + 1) + (page * Heigth), Chapter->ChapterIndex, Chapter->ChapterIndex, Chapter->PicRange);
            InPageItemCount++;
        ...
        while (true) {
          ...
          *Choice = InterfaceCursor ( 2, Top, Interface->Symbol, InPageItemCount, &NowIndex, Heigth, page, ItemCount);
          ...
          if (((NowIndex) > InPageItemCount + page * Heigth - 1 && Chapter != NULL) || (NowIndex < (page) * Heigth && NowIndex != -1)) {
            page = NowIndex / Heigth;
            break;
          }
          if (*Choice != 0)
            ...
            if (GetFileSize(TempFile) > 0 && IsConnectedToInternet()) {
              ...
              if (Chapter != NULL) {
                ...
                for (int Index = 1; Index <= atoi (PicRange); Index++) {
                  ...
                  Print (" ==> 下載第 %3d 張漫畫!\n", Index);
                  RunCommand (WgetFile, PicUrl, FilePath);
                  Print (" ==> 下載漫畫完成!\n");
                  ProgressBarTop (Progress+=AddProgress,"Download");
...
void UpdateApp (DATABASE* DataBase) {
  ...
  Print (" ==> 正在檢查更新，請稍後..\n");
  RunCommand (WgetFile, Announcement, TempFile);
  Free (Announcement);
